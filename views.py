...
class StreamingExpressions(View):
    ...
    def get_gn_search(self, fq, selected_facets):
        ...
        self.map_to_field = 'processo_pai'
        ...
        fq_string = ''
        # Essa verificacao eh decorrencia de alguma mudanca que houve na montagem dos facets.
        # Rever e limpar o codigo
        ...
        # self.gn_search_se = search_se % (self.collection, fl, sort, fq_string, fq_exclude_parent, selected_facets)

        search_se = 'search(%s, qt="/export", q=*:*, fl="%s", sort="%s asc", %s %s)'
        self.gn_search_se = search_se % (self.collection, fl, sort, fq_string, selected_facets)
        ...
        gather_nodes = gather_nodes % (
            self.collection, self.gn_search_se, self.origin_field, self.map_to_field, self.origin_field)
    ...
    def get_gn_children_other_parents(self):
        """
        The method get_gn_search() dont return the nodes that are not root and are not
        children of the selected query. In other words, nodes that belongs to the query but
        are children of a parent that do not belongs to the query.
        Solr´s gatherNode function seems to have a bug than it is not possible to have the
        children nodes of the returned nodes of this function.
        """
        get_gn_children_other_parents_fq = self.gn_search_se.replace('fq=-', 'fq=')
        get_gn_cop_query = "select(complement( %s, sort( %s, by=\"node asc\"), on=\"numero_processo=node\"),numero_processo as node)"
        get_gn_cop_query = get_gn_cop_query % (get_gn_children_other_parents_fq, self.gather_nodes)

        # import pdb; pdb.set_trace()
        return get_gn_cop_query


    def get_gn_totalization(self, gn_se, temporal_facet):
        # import pdb; pdb.set_trace()
        """
        ...
        """
        if temporal_facet:
            gn_totalization = 'rollup(having(sort(fetch(bv_collection, %s ,' \
            ...
            gn_totalization = gn_totalization % (gn_se, temporal_facet, temporal_facet, temporal_facet)
            ...
            gn_totalization = 'rollup(having(sort(fetch(graph_auxilios, %s ,' \
                              'fl="fomento_status_facet, valor_concedido", on="node=numero_processo",),' \
                              'by="level asc, fomento_status_facet asc"),gt(if(eq(valor_concedido,null), 0, valor_concedido),0)), over="level, fomento_status_facet", sum(valor_concedido), count(*))'
            gn_totalization = gn_totalization % (gn_se)
...
class SolrQueries(LoginRequiredMixin, View):
    ...
    def executaStreamingExpression(self, se, large_output=False):
        ...
        if large_output:
            ...
            with tempfile.NamedTemporaryFile(mode='w+', delete=False, suffix='.json', dir=settings_sf.DOWNLOAD_FILES) as temp:
                ...
                print 'End writing Solr SE response to NamedTemporaryFile'
                return temp
        else:
            try:
        ...
        for key, value in selected_facets.iteritems():
            ...
            for item in value:
                ...
                if not isinstance(item, int):
                    ...
                    simple_list.append(key + ':' + item)
                else:
                    facets += str(item) + ' '
                    simple_list.append(key + ':' + str(item))
            ...
            if item == '*':
                ...
                simple_list.append(key + ':*')

            lot.append(('fq', facets))
            se_fqs += se_fq
            #import pdb; pdb.set_trace()
        return ({'selected_facets_json':lot, 'selected_facets_se':se_fqs, 'simple_list':simple_list})

    def get_facets_json_api(self, conf_facets='' ):
        """
        ...
        json_facet = '{'
        if not conf_facets:
            conf_facets = COLLECTIONS[self.collection]['FACETS']
        for group in conf_facets:
    ...
    def get_content(self, content_type, fq, selected_facets, q='*:*', sort="score desc", rows="10", start="0", df="text", facet_fields='',proc_facet=True, group_list=[], group_limit="3", **kwargs):

        """
        Entender o que eh isso e ver se dah para melhorar.
        Tem a ver com mostrar facets ou nao, mas estah muito baguncado.
        """
        # Se facet_fields esta definodo e processamento de facet é igual a true
        if facet_fields and proc_facet:
            facet_fields = self.get_facets_json_api(facet_fields)[1]
        # Se facet_fields esta definodo e processamento de facet é igual a false
        elif facet_fields and not proc_facet:
            # Não altera o parametro
            pass
        elif (not facet_fields and not proc_facet):
            facet_fields = ''
        else:
        ...
        data = [('q', q), ('fl', '*'), ('fq', fq), ('df',df), ('json.facet', facet_fields), ('sort', sort), ('rows', rows), ('start', start)] + selected_facets + group_list
        ...
        # asdf = json.loads(response.content)
        # import pdb; pdb.set_trace()
        if response.status_code != 200:
            print "Erro no request para o Solr %s" % (response.status_code)
            self.solr_response_error('get_content', response, solr_url)

        return response.json()


    def get_content_generic(self, **kwargs):
        ...
        for key, param in kwargs.iteritems():
            ...
            #adiciona suport a grupos se vier em uma lista de tuplas sob o parametro group_list
            if key == 'group_list':
                for group in param:
                    parametros_toSolr.append((group[0], group[1]))
    ...
    def get_content_json_facet(self, content_type, fq, json_facet, selected_facets, rows=0, offset=0, limit=10):
        """
        ...
        try:
            data = [('q', '*:*'), ('rows', rows), ('fq', fq), ('json.facet', json_facet)] + selected_facets
        except:
            logger.error('Possivel erro na concatenacao de lista')
            print "\n\nPossivel erro na concatenacao de lista - solr_front.py"
            raise
        solr_url = solr_url.replace('%', '%25')
        # import pdb; pdb.set_trace()
        response = requests.post(solr_url, data=data)

        # print json.loads(response.content)['facets']['nivel_1']['buckets'][0]
        # print json.loads(response.content)['facets']['nivel_1']['buckets'][1]
        # import pdb; pdb.set_trace()
...
class EntryPointView(LoginRequiredMixin, View):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        # Trying access to a not known collection.
        if not kwargs['collection'] in COLLECTIONS:
            raise Http404("Collection does not exist: %s" % kwargs['collection'])


        # para não sobrescrever templates que ja foram definidos nas views
        if hasattr(self, 'template_name') and 'template' in kwargs:
        ...
        self.vertices = GRAPH[self.collection]
        # condicional possibilita sobrescrever dado antes da chamada super do dispatch
        if not hasattr(self, 'body_json'):
        ...
        try:
            if hasattr(self, 'json_response'):
                solr_json = self.json_response()
                return JsonResponse(solr_json)
            else:
                return super(EntryPointView,self).dispatch(*args, **kwargs)

            # elif hasattr(self, 'html_response'):
            #     # metodo deve chamar get_object se for o caso,
            #     # get_template_names e get_context_data
            #     # e retornar um httpresponse
            #     return self.html_response(*args, **kwargs)
            # else:
            #     raise NotImplementedError("NotImplementedError: Nem json_response ou html_response foram implementados na class %s" % self.__class__)
        except GetSolarDataException:
...
class GatherNodesView(EntryPointView):
    ...
    def json_response(self):
        ...
        try:
            ...
            #     self.retorno = self.se.get_gn_totalization(self.se.gather_nodes, self.temporal_facet)
            #
            ...
            retorno = self.se.get_gn_totalization(self.se.gather_nodes, 0)

            """
            gn_children_other_parents = self.se.get_gn_children_other_parents()
            retorno_children_other_parents = self.se.get_gn_totalization(gn_children_other_parents, 0)
            # import pdb; pdb.set_trace()
            if retorno_children_other_parents:
                retorno += retorno_children_other_parents
            else:
                retorno += ' '
            """
...
class ParamsView(LoginRequiredMixin, TemplateView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        if not self.vertice:
            ...
            try:
                #quando não é uma pagina do buscador da pau, pq reverse depende da pagina conter o padrão de url do solr_front
...
class ExportDataView(EntryPointView):
    ...
    def json_response(self):
        ...
        if form.is_valid():
            ...
            # Caso especifico da collection graph_auxilios. Auxilios e bolsas tem um campo
            # que jah tem tudo concatenado.
            """
        ...
        if 'export_fields' in COLLECTIONS[self.vertice['collection']]['EXPORT_DATA']:
            ...
            if 'max_rows' in COLLECTIONS[self.vertice['collection']]['EXPORT_DATA']:
                try:
                    ...
                    #data_list = self.solr_queries.executaStreamingExpression(se)
                except GetSolarDataException:
                    return HttpResponseServerError()
            else:
                try:
            ...
            if settings_sf.USE_CELERY:
                try:
                    #makeData_celery.delay(data_list.name, nome, email, para, msg, fields, 'csv', column_names)
...
class AjaxDocsWidget(EntryPointView):
    ...
    def get_solrquery(self, **kwargs):
        ...
        #remover esta instanciação, pois entrypoint já possui ela

        return self.solr_queries.get_content_generic(**kwargs)
