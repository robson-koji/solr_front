...
class StreamingExpressions(View):
    ...
    def get_gn_search(self, fq, selected_facets):
        ...
        gather_nodes = gather_nodes % (self.collection, search_se, self.origin_field, self.map_to_field, self.origin_field)
        ...
        def rec_gather_nodes(graph_depth, gather_nodes):
            ...
            if graph_depth > 0:
                ...
                gather_nodes_add_level = gather_nodes_add_level % (self.collection, gather_nodes, self.map_to_field, self.origin_field)
        ...
        if 'json_facet' in self.data and self.data['json_facet'] != '':
            ...
            try:
                solr_json = self.solr_queries.get_content_json_facet(self.collection, self.fq, json_facet,
                                                                     self.selected_facets['selected_facets_json']).json()
            except GetSolarDataException:
                raise
        else:  # Main request
            try:
                solr_json = self.solr_queries.get_content(self.collection, self.fq, self.selected_facets['selected_facets_json'])
            except GetSolarDataException:
                raise
            ...
            if 'facet' in totalizador:
                ...
                try:
                    ...
                    totalizador_dict['numFound'] = self.solr_queries.get_facets_json_api_unique(totalizador['unique'],
                                                                                                self.fq,
                                                                                                self.selected_facets['selected_facets_se'])
                except GetSolarDataException:
                    raise
    ...
    def get(self, request, *args, **kwargs):
        ...
        if 'collection_destino' in kwargs:
            ...
            self.id = self.navigate.add_vertice(kwargs['collection_destino'], int(kwargs['id']), {
                'qs_selected_facets': {self.solr_queries.campo_dinamico_busca: int(self.solr_queries.hash_querybuilder)}},
