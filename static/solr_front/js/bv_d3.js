            ...
            */
            $.each(todos_nivel_2, function (id, elemen) {
                for (var j = 0; j < json_d3.length; j++) {
                    ...
                    if (!existe) {
                        json_d3[j].data[k] = {'count': 0, 'val': elemen}
                    }
                }
            })
        })
...
function AnualChart(id, label, json) {
    ...
    x.domain(refaz_data.map(function (d) {
        return d.value.ano;
    }));
    y.domain([0, d3.max(refaz_data, function (d) {
        return d.value.valor;
    })]);
    ...
    svg.selectAll("bar")
        ...
        .attr("x", function (d) {
            return x(d.value.ano);
        })
        .attr("width", x.rangeBand())
        .attr("y", function (d) {
            return y(d.value.valor);
        })
        .attr("height", function (d) {
            return height - y(d.value.valor);
        });
...
function MensalChart(id, label, json) {
    ...
    x.domain(d3.extent(refaz_data, function (d) {
        return d.value.ano;
    }));
    y.domain(d3.extent(refaz_data, function (d) {
        return d.value.valor;
    }));
    ...
    yticksMax = 20
    rangeMax = d3.max(refaz_data, function (d) {
        return d.value.valor;
    })
    ...
    svg.selectAll("dot")
        ...
        .attr("r", 1.5)
        .attr("cx", function (d) {
            return x(d.value.ano) + margin.left
        })
        .attr("cy", function (d) {
            return y(d.value.valor) + margin.top;
        })
        .style('fill', '#e44998')
...
function barChart(id, label, json) {
    ...
    if (refaz_data.length) {
        ...
        var rects = group.append('rect')
            ...
            .attr('y', function (d, i) {
                return (altura_bar * i) + altura_bar + (10 * i)
            })

        var texts = group.append('text')
            .text(function (d) {
                return d.value.valor + ' - ' + d.value.label;
            })
            .attr("font-family", "roboto  ")
            .attr("font-size", "12px")
            .attr("y", function (d, i) {
                return (altura_bar * i) + altura_bar + (10 * i) + 17
            })
