...
function makeDistroChart(settings) {
    ...
    !function prepareData() {
        function calcMetrics(values) {
            ...
            metrics.quartile1 = values[0].quartile1;
            metrics.median = values[0].median;
            metrics.count = values[0].count;
            metrics.mean = values[0].mean;
        ...
        function kernelDensityEstimator(kernel, x) {
            return function (sample) {
                return x.map(function (x) {
                    return {
                        x: x, y: d3.mean(sample, function (v) {
                            return kernel(x - v);
                        })
                    };
                });
    ...
    chart.renderNotchBoxes = function (options) {
        ...
        !function calcNotches() {
            ...
            for (var cName in chart.groupObjs) {
                ...
                modifier = (1.57 * (cNotch.metrics.iqr / Math.sqrt(cNotch.values[0].count)));
